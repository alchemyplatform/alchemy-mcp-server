...
export function createMcpServer(): McpServer {
  ...
  server.tool(
    ...
    async (params) => {
      try {
        let result = await alchemyApi.getTransactionHistoryByMultichainAddress(
          params
        );
        ...
        if (error instanceof Error) {
          console.error(
            "Error in getTransactionHistoryByMultichainAddress:",
            error
          );
          return {
            content: [{ type: "text", text: `Error: ${error.message}` }],
  ...
  server.tool(
    "fetchTransfers",
    {
      ...
      category: z
        ...
        .describe('The category of transfers to query. e.g. "external" or "internal"'),
      order: z
        .string()
        .default("asc")
        ...
        .array(
          z.object({
            ...
            spamConfidenceLevel: z
              ...
              .default("VERY_HIGH")
              .describe('The spam confidence level to query. e.g. "LOW" or "HIGH"'),
          })
        )
        .describe("A list of wallet address and network pairs"),
      withMetadata: z
  ...
  // Fetch the contract data for an NFT owned by a singular or multiple wallet addresses
  server.tool(
