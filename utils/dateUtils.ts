...
export function calculateDateRange(timeFrame: string): DateRange {
  ...
  if (
    ["today", "yesterday", "last-week", "last-month", "start-of-year"].includes(
  ...
  // Handle trailing periods (last 7 days, etc.)
    ...
    normalizedTimeFrame === "last-7-days" ||
    normalizedTimeFrame === "last-week"
  ) {
    startDate.setDate(now.getDate() - 7);
  }
  // Handle past-Nd format (days)
  else if (
    normalizedTimeFrame.startsWith("past-") &&
    normalizedTimeFrame.endsWith("d")
  ) {
    const days = parseInt(
      normalizedTimeFrame.substring(5, normalizedTimeFrame.length - 1),
    );
    if (!isNaN(days)) {
  ...
  // Handle past-Nw format (weeks)
    ...
    normalizedTimeFrame.startsWith("past-") &&
    normalizedTimeFrame.endsWith("w")
  ) {
  ...
  // Handle past-Nm format (months)
    ...
    normalizedTimeFrame.startsWith("past-") &&
    normalizedTimeFrame.endsWith("m")
  ) {
  ...
  // Handle past-Ny format (years)
    ...
    normalizedTimeFrame.startsWith("past-") &&
    normalizedTimeFrame.endsWith("y")
  ) {
  ...
  // Default to 7 days if format not recognized
    ...
    startDate.setDate(now.getDate() - 7);
    console.warn(
      `Unrecognized timeFrame format: ${timeFrame}. Defaulting to past 7 days.`,
    );
  }
...
export function parseNaturalLanguageTimeFrame(query: string): string {
  ...
  if (
    ...
    normalizedQuery.includes("previous week")
  ) {
    return "last-week";
  } else if (
    ...
    normalizedQuery.includes("previous year")
  ) {
    return "past-1y";
  } else if (
    normalizedQuery.includes("this year") ||
    normalizedQuery.includes("year to date") ||
    normalizedQuery.includes("ytd")
  ) {
    return "ytd";
  } else if (
    normalizedQuery.includes("this month") ||
    normalizedQuery.includes("month to date") ||
    normalizedQuery.includes("mtd")
  ) {
    return "mtd";
  } else if (
    normalizedQuery.includes("this quarter") ||
    normalizedQuery.includes("quarter to date") ||
    normalizedQuery.includes("qtd")
  ) {
    return "qtd";
